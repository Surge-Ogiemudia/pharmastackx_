...
const verifiedImages = {
  ...
  gyno_daktarin: "https://d3ckuu7lxvlwp2.cloudfront.net/products/QWMhy3Ab7Ub1XeX5EYK0KoHPC4aHNVYblGXLD74r.jpeg",
...
function assignProductImage(product) {
  const norm = (product.name + ' ' + (product.activeIngredient || '')).toLowerCase();
...
const prods = JSON.parse(fs.readFileSync('scripts/bubblegum_products.json', 'utf8'));
let unassigned = 0;
let assigned = 0;

prods.forEach(p => {
  const img = assignProductImage(p);
  if (!img) {
    unassigned++;
    console.log('UNASSIGNED:', p.name, '|', p.category, '|', p.activeIngredient);
  } else {
    assigned++;
  }
});

console.log(`Summary: ${assigned} assigned, ${unassigned} unassigned out of ${prods.length}`);
